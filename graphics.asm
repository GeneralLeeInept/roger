	; Dlist - 240 scanlines
	.align $400
	.local dlist
	FS	= $10
	VS	= $20
	LMS	= $40
	DLI	= $80

	; Overscan (16 scanlines)
	.byte $70, $70
	
	; Score area - 1 mode 2 line (8 scanlines)
	.byte $02 + LMS + DLI, a(screenmem)
	
	; Goal area - 3 mode 4 lines (24 scanlines)
	.byte $04, $04, $04 + DLI
	
	; River - 10 mode 4 lines (80 scanlines)
	.byte $04 + FS + LMS
river1	.byte a(screenmem.river1)
	.byte $04 + FS + LMS + DLI
river2	.byte a(screenmem.river2)
	.byte $04 + FS + LMS
river3	.byte a(screenmem.river3)
	.byte $04 + FS + LMS + DLI
river4	.byte a(screenmem.river4)
	.byte $04 + FS + LMS
river5	.byte a(screenmem.river5)
	.byte $04 + FS + LMS + DLI
river6	.byte a(screenmem.river6)
	.byte $04 + FS + LMS
river7	.byte a(screenmem.river7)
	.byte $04 + FS + LMS + DLI
river8	.byte a(screenmem.river8)
	.byte $04 + FS + LMS
river9	.byte a(screenmem.river9)
	.byte $04 + FS + LMS
river10	.byte a(screenmem.river10)

	;.byte $41, .LO(dlist), .HI(dlist)

	; River bank - 2 mode 4 lines (16 scanlines)
	.byte $04 + LMS, a(screenmem.riverbank)
	.byte $04 + DLI
	
	; Road - 8 mode 4 lines (64 scanlines)
	.byte $04 + FS + LMS, a(screenmem.road1)
	.byte $04 + FS + LMS, a(screenmem.road2)
	.byte $04 + FS + LMS + DLI, a(screenmem.road3)
	.byte $04 + FS + LMS, a(screenmem.road4)
	.byte $04 + FS + LMS, a(screenmem.road5)
	.byte $04 + FS + LMS + DLI, a(screenmem.road6)
	.byte $04 + FS + LMS, a(screenmem.road7)
	.byte $04 + FS + LMS, a(screenmem.road8)
	
	; Footpath - 4 mode 4 lines (32 scanlines)
	.byte $04 + LMS, a(screenmem.footpath)
	.byte $04, $04, $04
	
	; End
	.byte $41, a(dlist)
	
	.endl ; dlist

	; Screen memory
	.align $1000
	.local screenmem
scoreline
	;.byte "        SCORE 9999    LIVES 9    HI 9999       "
	.byte "    0123456789012345678901234567890123456789 "
goalarea
	.he 00 00 00 00 00 00 01 02 01 02 05 06 01 02 01 02 01 02 05 06 01 02 01 02 01 02 05 06 01 02 01 02 01 02 05 06 01 02 01 02 01 02 05 06 01 02 01 02
	.he 00 00 00 00 00 00 01 02 01 0a 00 00 0b 02 01 02 01 0a 00 00 0b 02 01 02 01 0a 00 00 0b 02 01 02 01 0a 00 00 0b 02 01 02 01 0a 00 00 0b 02 01 02 
	.he 00 00 00 00 00 00 05 06 0d 0e 00 00 0f 10 05 06 0d 0e 00 00 0f 10 05 06 0d 0e 00 00 0f 10 05 06 0d 0e 00 00 0f 10 05 06 0d 0e 00 00 0f 10 05 06
	
	.align $100
river1	.he 19 1a 1b 1c 19 1a 1b 1c 19 1a 1b 1c 19 1a 1b 1c 00 00 11 12 13 14 11 12 13 14 11 12 13 14 11 12 13 14 00 00 19 1a 1b 1c 19 1a 1b 1c 19 1a 1b 1c 19 1a 1b 1c 00 00 11 12 13 14 11 12 13 14 11 12 13 14 11 12 13 14 00 00
river1_end
	.ds 60

	.align $100
river2	.he 1d 1e 1f 20 1d 1e 1f 20 1d 1e 1f 20 1d 1e 1f 20 00 00 15 16 17 18 15 16 17 18 15 16 17 18 15 16 17 18 00 00 1d 1e 1f 20 1d 1e 1f 20 1d 1e 1f 20 1d 1e 1f 20 00 00 15 16 17 18 15 16 17 18 15 16 17 18 15 16 17 18 00 00
river2_end
	.ds 60
	
	.align $100
river3	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
river3_end
	.ds 60
	
	.align $100
river4	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
river4_end
	.ds 60

	.align $100
river5	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00
	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00,$00
	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00
river5_end
	.ds 60
	
	.align $100
river6	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
river6_end
	.ds 60
		
	.align $100
river7	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
river7_end
	.ds 60

	.align $100
river8	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00
	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00,$00
	.byte $01,$02,$03,$04,$05,$06,$07,$08,$09,$0a,$0b,$00,$00
river8_end
	.ds 60
	
	.align $100
river9	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
river9_end
	.ds 60
	
	.align $100
river10	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00,$00
	.byte $0c,$0d,$0e,$0f,$10,$11,$12,$13,$14,$15,$16,$00,$00
river10_end
	.ds 60
	
	.align $100
riverbank
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00

	.align $100
road1	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.byte $17,$18,$19,$1a,$00,$00,$20,$21,$22,$23,$00
	.ds 60

road2	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.byte $1b,$1c,$1d,$1e,$00,$00,$24,$25,$26,$27,$00
	.ds 60

road3	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
road4	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
road5	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
road6	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00	
road7	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
road8	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00

	.align $100
footpath
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00
	.byte $00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00,$00

	.endl ; screenmem
	